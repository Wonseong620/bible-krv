...
## 오늘의 주제·명사형 키워드 TOP 7

GET `/api/trends`는 한국시간 당일의 주제·키워드를 각각 최대 7개 반환한다.
`trends.py`의 고정 어휘 목록에만 매핑하며 자유 텍스트·이름·연락처를 추출하거나 저장하지 않는다.
첫 요청에 정상 답변이 완료될 때 날짜·구분·허용된 라벨·건수만 quota DB에 함께 커밋한다.
후속 요청·오류는 집계하지 않는다. 각 첫 대화의 동일 라벨은 1회만 집계한다.
5개 이상 대화에 등장한 항목만 빈도순으로 공개하고 정확한 건수는 API에 포함하지 않는다.
일일 주제 순위이며 고유한 사람의 수를 의미하지 않는다. 로그인 없이 새 대화를 반복하면 중복 집계될 수 있다.
과거 원문을 보관하지 않으므로 이전 대화의 소급 집계는 하지 않는다.
현재 분류는 단순한 주제·키워드 사전 기반이며 문맥·부정 표현 등을 정확히 이해하는 의미 분석은 아니다.
실제 집계가 없으면 빈 상태를 표시한다. 예시나 테스트 데이터를 운영 DB에 넣지 않는다.
